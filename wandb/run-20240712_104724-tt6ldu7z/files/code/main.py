...
def run(args):
    ...
    init_model = copy.deepcopy(model) # To use l2_init
    ...
    if args.train_type == 'cold':
        wandb.run.name = f"{args.optimizer_type.capitalize()}_Cold {str(args.seed)[-1]}"
    elif args.train_type == 'warm':
        wandb.run.name = f"{args.optimizer_type.capitalize()}_Warm {str(args.seed)[-1]}" 
    elif args.train_type == 'warm_rm':
        wandb.run.name = f"{args.optimizer_type.capitalize()}_WarmReM {str(args.seed)[-1]}"  
    elif args.train_type == 'sp':
        wandb.run.name = f"{args.optimizer_type.capitalize()}+S&P({args.sp_lambda}) {str(args.seed)[-1]}"
    elif args.train_type == 'dash':
        wandb.run.name = f"{args.optimizer_type.capitalize()}+DASH({args.dash_lambda}/{args.dash_alpha}) {str(args.seed)[-1]}"

    elif args.train_type == 'l2_init':
        wandb.run.name = f"{args.optimizer_type.capitalize()}+L2_INIT({args.l2_init_lambda}) {str(args.seed)[-1]}"
    elif args.train_type == 'reset':
        wandb.run.name = f"{args.optimizer_type.capitalize()}+Reset {str(args.seed)[-1]}"
    ...
    for chunk_idx in tqdm.tqdm(range(args.num_chunks)):
        ...
        if (args.train_type == 'dash') & (chunk_idx >= 1):
            ...
            model, new_grad_l2_norm = dash(args, model, criterion, recent_chunk_loader)
        ...
        while not done_with_chunk:
            ...
            if args.sota == True:
                scheduler.step()
            # break if converge
            if (train_acc >= converge_acc) | (epoch >= args.max_epoch):
                break
...
if __name__=='__main__':
    ...
    if 'mlp' in args.model:
        args.project_name = f'{args.model}-{args.dataset}-d{args.depth}-w{args.width}-online'
    else:
        args.project_name = f'{args.model}-{args.dataset}-online'
