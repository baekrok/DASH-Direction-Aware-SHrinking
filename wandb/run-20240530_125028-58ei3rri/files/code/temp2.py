...
def train(args):
    wandb.init(project="cifar10-cnn2")
    ...
    # Split the training data into two chunks
    chunks = random_split(trainset, [len(trainset) // 2, len(trainset) - len(trainset) // 2])

    # Training loop
    optimizer = torch.optim.SGD(model.parameters(), momentum=0.9, lr=0.01)
    args.num_epochs = args.start_epoch + 40 + 100
    for epoch in range(args.num_epochs):
        if args.start_epoch <= epoch < args.start_epoch + 40:
            # Switch to training on 50% of the data
            chunk_loader = DataLoader(chunks[0], batch_size=128, shuffle=True)
        else:
            chunk_loader = train_loader
        ...
        print(f"Epoch [{epoch + 1}/{args.num_epochs}], Loss: {running_loss / len(chunk_loader):.4f}, "
              f"Train Accuracy: {correct_train / total_train:.4f}, "
              f"Test Accuracy: {val_accuracy:.4f}")
        wandb.log({"epoch": epoch, "loss": running_loss / len(chunk_loader),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_epoch", type=int, default=50, help="Epoch to start training with 50% of the data")
