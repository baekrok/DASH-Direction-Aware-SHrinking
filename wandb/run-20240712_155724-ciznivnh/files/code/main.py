...
def run(args):
    ...
    for chunk_idx in range(args.num_chunks):
        ...
        ########################
        ## Train
        train_acc, train_loss, step = train_chunk(args, model, init_model, criterion, optimizer, chunk_loader, chunk_idx)
        
        # Evaluate
        logs = evaluate(args, test_loader, model, train_acc, train_loss, step)
...
def train_chunk(args, model, init_model, criterion, optimizer, chunk_loader, chunk_idx):
    model.train()
    ...
    while True:
        ...
        if chunk_idx >= 1:
            prev_log = get_prev_metric(args, epoch, chunk_idx, model, 
                                       prev_chunk_loader, chunk_loader, current_chunk_loader)

        epoch_acc, epoch_loss = train_epoch(args, model, init_model, criterion, optimizer, chunk_loader, epoch)
    ...
    return train_acc/epoch, train_loss/epoch, step

def train_epoch(args, model, init_model, criterion, optimizer, chunk_loader, epoch):
